...
def mock_knuspr_client():
    client = AsyncMock()
    client.create_cart.return_value = MagicMock(
        spec=KnusprCart, cart_id="cart_123", total_price=100.0
    )
...
def mock_ingredient_mapper():
    ...
    mapper.categorize_products = MagicMock(
        return_value={
            "dairy": [{"name": "Milk", "quantity": 1, "unit": "l", "price": 2.5}]
        }
    )
    return mapper
...
async def test_create_cart_success_emits_event_and_metrics(
    ...
    # Mock metrics
    with patch("app.agents.cart_optimizer.cart_creation_total") as mock_total, patch(
        "app.agents.cart_optimizer.cart_creation_duration_seconds"
    ) as mock_duration, patch(
        "app.agents.cart_optimizer.cart_value_eur"
    ) as mock_value, patch(
        "app.agents.cart_optimizer.cart_items_count"
    ) as mock_count:

        # Execute
        result = await agent.create_cart_from_meal_plan(
            meal_plan_id=123, user_id=456, credential_manager=MagicMock()
        )
        ...
        assert event.payload["meal_plan_id"] == 123
